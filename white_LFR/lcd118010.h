...
DESCRIPTION: 
	...
	lcd_init() ----- Call the lcd_init() function before calling any other functionsto initialize the lcd.

	lcd_cmd() ------ This function is used to give any command instructions to the LCD. For e.g lcd_cmd(0x01) will give the 
	 command.
	...
	lcd_gotoxy()--- Will set the cursor at a particular position on of the LCD.
			  So lcd_gotoxy(1,2) will set the cursor at the 2d row 3rd column
...
{	  
	...
	lcd_cmd(0x28);		//Set interface 40 bit 2 line
	delay_ms_lcd(1);
